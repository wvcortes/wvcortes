...
export default async function Servicos() {
  ...
  return <><SiteHeader barbearia={barbearia} usuario={usuario}/><main>
    <section className="bg-tinta px-5 py-16 text-marfim sm:py-24"><div className="mx-auto max-w-7xl">
      ...
      <p className="mt-5 max-w-xl text-marfim/60">Escolha seu cuidado e reserve o melhor horário para você.</p>
    </div></section>
    <div className="secao">{categorias.map((cat) => <section key={cat} className="mb-16 last:mb-0">
      ...
      <div className="mt-7 grid gap-5 md:grid-cols-2">{servicos.filter((s) => (s.categoria || "Barbearia") === cat).map((s) =>
        <article key={s.id} className="card-premium flex flex-col p-6 sm:p-7">
          ...
          <div className="mt-6 flex items-center justify-between border-t border-linha pt-4"><span className="etiqueta text-fumaca">{s.duracao_min} minutos</span><Link href="/agendar" className="text-sm font-semibold text-couro">Agendar →</Link></div>
        </article>)}</div>
    </section>)}</div>
