...
export async function POST(req) {
  try {
    ...
    const envio = await db.storage.from("colaboradores").upload(path, arquivo, { contentType: arquivo.type, upsert: false });
    if (envio.error) return NextResponse.json({ erro: envio.error.message }, { status: 400 });
    const { data } = db.storage.from("colaboradores").getPublicUrl(path);
...
export async function DELETE(req) {
  try {
    ...
    const prefixo = `${String(process.env.NEXT_PUBLIC_SUPABASE_URL || "").replace(/\/$/, "")}/storage/v1/object/public/colaboradores/`;
    if (typeof url !== "string" || !url.startsWith(prefixo)) {
      return NextResponse.json({ erro: "A foto informada não pertence ao bucket de colaboradores." }, { status: 400 });
    }

    const path = decodeURIComponent(url.slice(prefixo.length));
    if (!path || path.includes("..")) return NextResponse.json({ erro: "Caminho de foto inválido." }, { status: 400 });
    const removido = await db.storage.from("colaboradores").remove([path]);
